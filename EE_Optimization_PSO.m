...
iterations=1;
NL=1;          % # Realizations          
K_dB=20;         % K factor [dB] for the Rician Channel Model
SNR=0;          % SNR [dB]
Nm=[16];         % Number of Antennas per UT
...
for nm=1:length(Nm)
    ...
    WW_norm=zeros(length(P_max),UT,NL,iterations);
    WZF_norm=zeros(length(P_max),UT,NL,iterations);
...
%     diag_Q_m=diag(Q_m);
    ...
    for m=1:UT
        ...
        H_Dm{m}=Ray_model(B,Nm(nm),L);                  % Channel UT-BS
    end
    ...
    for pp=1:length(P_max)
        ...
        Wm0W={};
        Wm0WZF={};
        Wm0{1,1} = cell2mat(precMAtrix(H_m,1));
        Wm0W{1,1} = cell2mat(precMAtrix(H_m,2));
        Wm0WZF{1,1} = cell2mat(precMAtrix(H_m,3));
        

        %% Initial Random solution AND Max Power Transmit
        % Criterio w/o prec
        [~,rho_EE(pp,1,nn,:),rho_SE(pp,1,nn,:)]= rho_fun_PSO(H_Dm,H_B,Phi,H_Am,Q_m,1,'EE',SNR,Nr,BW,UT,xi,P_max(pp),Pcm,P_BS,P_RIS);
        % Criterio SVD
        [~,rho_EEW(pp,1,nn,:),rho_SEW(pp,1,nn,:)]= rho_fun_PSO(H_Dm,H_B,Phi,H_Am,Q_m,2,'EE',SNR,Nr,BW,UT,xi,P_max(pp),Pcm,P_BS,P_RIS);
        % Criterio prec-ZF
        [~,rho_EEWZF(pp,1,nn,:),rho_SEWZF(pp,1,nn,:)]= rho_fun_PSO(H_Dm,H_B,Phi,H_Am,Q_m,3,'EE',SNR,Nr,BW,UT,xi,P_max(pp),Pcm,P_BS,P_RIS);
        ...
        %%    Phi Optimization
            ...
            problem2.cost  = @(x) rho_fun_PSO(H_Dm,H_B,x,H_Am,Q_m,1,'EE',SNR,Nr,BW,UT,xi,P_max(pp),Pcm,P_BS,P_RIS);    
            [theta_new, ~, ~, ~] = pso(problem2, Phi0, options);
            Phi0=mat2cell(theta_new,Nr);
            [~,rho_EE(pp,3,nn,optz_iter),rho_SE(pp,3,nn,optz_iter)]=rho_fun_PSO(H_Dm,H_B,theta_new,H_Am,Q_m,1,'EE',SNR,Nr,BW,UT,xi,P_max(pp),Pcm,P_BS,P_RIS);    
            time_wo=toc+time_wo;
            % Criterio B
            tic
            problem2W.M = complexcirclefactory(Nr);
            problem2W.cost  = @(x) rho_fun_PSO(H_Dm,H_B,x,H_Am,Q_m,2,'EE',SNR,Nr,BW,UT,xi,P_max(pp),Pcm,P_BS,P_RIS);    
            [theta_newW, ~, ~, ~] = pso(problem2W, Phi0W, options);
                Phi0W=mat2cell(theta_newW,Nr);
            [~,rho_EEW(pp,3,nn,optz_iter),rho_SEW(pp,3,nn,optz_iter)]=rho_fun_PSO(H_Dm,H_B,theta_newW,H_Am,Q_m,2,'EE',SNR,Nr,BW,UT,xi,P_max(pp),Pcm,P_BS,P_RIS);    
            time_svd=toc+time_svd;
            % Criterio prec-ZF
            tic
            problem2WZF.M = complexcirclefactory(Nr);
            problem2WZF.cost  = @(x) rho_fun_PSO(H_Dm,H_B,x,H_Am,Q_m,3,'EE',SNR,Nr,BW,UT,xi,P_max(pp),Pcm,P_BS,P_RIS);    
            [theta_newWZF, ~, ~, ~] = pso(problem2WZF, Phi0WZF, options);
                Phi0WZF=mat2cell(theta_newWZF,Nr);
            [~,rho_EEWZF(pp,3,nn,optz_iter),rho_SEWZF(pp,3,nn,optz_iter)]=rho_fun_PSO(H_Dm,H_B,theta_newWZF,H_Am,Q_m,3,'EE',SNR,Nr,BW,UT,xi,P_max(pp),Pcm,P_BS,P_RIS);    
            time_zf=toc+time_zf;
          %% W optz
            
            % Criterio w/o prec
            tic
            problem3.M = spherecomplexfactory(Nm(nm),UT*Nm(nm)); % obliquecomplexfactory  euclideancomplexfactory
            problem3.cost  = @(x) rho_fun_PSO(H_Dm,H_B,theta_new,H_Am,Q_m,1,'EE',SNR,Nr,BW,UT,xi,P_max(pp),Pcm,P_BS,P_RIS,x);
            [W_m_new, ~, ~, ~] = pso(problem3, Wm0, options);
            Wm0{1,1}=(W_m_new);
            W_m_cell=mat2cell(W_m_new,Nm(nm),ones(1,UT)*Nm(nm));
            for uu=1:UT
                W_norm(pp,uu,nn,optz_iter)=norm(W_m_cell{uu},2);
            end
            [~,rho_EE(pp,4,nn,optz_iter),rho_SE(pp,4,nn,optz_iter)]=rho_fun_PSO(H_Dm,H_B,theta_new,H_Am,Q_m,1,'EE',SNR,Nr,BW,UT,xi,P_max(pp),Pcm,P_BS,P_RIS,W_m_new);
            time_wo=toc+time_wo;
            % Criterio B
            tic
            problem3W.M = spherecomplexfactory(Nm(nm),UT*Nm(nm)); % obliquecomplexfactory  euclideancomplexfactory
            problem3W.cost  = @(x) rho_fun_PSO(H_Dm,H_B,theta_newW,H_Am,Q_m,2,'EE',SNR,Nr,BW,UT,xi,P_max(pp),Pcm,P_BS,P_RIS,x);
            [W_m_newW, ~, ~, ~] = pso(problem3W, Wm0W, options);
            Wm0W{1,1}=(W_m_newW);
            W_m_cell=mat2cell(W_m_newW,Nm(nm),ones(1,UT)*Nm(nm));
            for uu=1:UT
                WW_norm(pp,uu,nn,optz_iter)=norm(W_m_cell{uu},2);
            end
            [~,rho_EEW(pp,4,nn,optz_iter),rho_SEW(pp,4,nn,optz_iter)]=rho_fun_PSO(H_Dm,H_B,theta_newW,H_Am,Q_m,2,'EE',SNR,Nr,BW,UT,xi,P_max(pp),Pcm,P_BS,P_RIS,W_m_newW);
            time_svd=toc+time_svd;
            % Criterio prec-ZF
            tic
            problem3WZF.M = spherecomplexfactory(B,UT*Nm(nm)); % obliquecomplexfactory  euclideancomplexfactory
            problem3WZF.cost  = @(x) rho_fun_PSO(H_Dm,H_B,theta_newWZF,H_Am,Q_m,3,'EE',SNR,Nr,BW,UT,xi,P_max(pp),Pcm,P_BS,P_RIS,x);
            [W_m_newWZF, ~, ~, ~] = pso(problem3WZF, Wm0WZF, options);
            Wm0WZF{1,1}=(W_m_newWZF);
            W_m_cell=mat2cell(W_m_newWZF,B,ones(1,UT)*Nm(nm));
            for uu=1:UT
                WZF_norm(pp,uu,nn,optz_iter)=norm(W_m_cell{uu},2);
            end
            [~,rho_EEWZF(pp,4,nn,optz_iter),rho_SEWZF(pp,4,nn,optz_iter)]=rho_fun_PSO(H_Dm,H_B,theta_newWZF,H_Am,Q_m,3,'EE',SNR,Nr,BW,UT,xi,P_max(pp),Pcm,P_BS,P_RIS,W_m_newWZF);
            time_zf=toc+time_zf;
        end
...
% hold on
% plot(rho_SEW_mean)
legend('${\textsc{ee}^0_{\forall}(\vec{\Phi}^{\iota},\vec{W}^\iota)}$',...
    '${\textsc{ee}^{SVD}_{\forall}(\vec{\Phi}^{\iota},\vec{W}^\iota)}$',...
    '${\textsc{ee}^{ZF}_{\forall}(\vec{\Phi}^{\iota},\vec{W}^\iota)}$',...
    '${\textsc{ee}^0_{\forall}(\vec{\Phi}^{0},\vec{W}^0)}$',...
    '${\textsc{ee}^0_{\forall}(\vec{\Phi}^{\iota},\vec{W}^0)}$','Location','best','interpreter','latex');
...
% hold on
% plot(rho_SEW_mean)
legend('$\textsc{se}_{\vec{W}}(\textrm{Rndm})$ w/o prec','$\textsc{se}_{\vec{W}}$ w/o prec','$\textsc{se}_{\vec{W}}(\vec{\Phi})$ w/o prec','$\textsc{se}_{\forall}(\vec{W})$ w/o prec',...
    '$\textsc{se}_{\vec{W}}(\textrm{Rndm})$  prec-SVD','$\textsc{se}_{\vec{W}}$ prec-SVD',...
    '$\textsc{se}_{\vec{W}}(\vec{\Phi})$ prec-SVD','$\textsc{se}_{\forall}(\vec{W})$ prec-SVD',...
    '$\textsc{se}_{\vec{W}}(\textrm{Rndm})$  prec-ZF','$\textsc{se}_{\vec{W}}$  prec-ZF',...
...
for pm=1:length(P_max)
     ...
     text(rand_loc(pm),value_iter(rand_loc(pm)),"$ P_{\textrm{max},m}$ = " + string(P_max(pm))+" dBm",'interpreter','latex','FontSize',8,'BackgroundColor','w')
     hold on
 end
    hold off
    set(gca,'XMinorTick','on','YMinorTick','on');
...
ylabel(' ${\textsc{ee}^0_{\forall}(\vec{\Phi}^{\iota},\vec{W}^\iota)}$ (bits/Joule)','interpreter','latex');
% plot(rho_EEW_mean)
% hold on
% plot(rho_SEW_mean)
...
% legendStrings = "$ P_{\textrm{max},m}$ = " + string(P_max)+" dBm";
xlabel('Iteration #','Interpreter','latex');
ylabel(' ${\textsc{ee}^\textrm{SVD}_{\forall}(\vec{\Phi}^{\iota},\vec{W}^\iota)}$ (bits/Joule)','interpreter','latex');
% plot(rho_EEW_mean)
% hold on
% plot(rho_SEW_mean)
% legend(legendStrings,'Location','bestoutside','interpreter','latex');
...
grid on
xlabel('$ P_{\textrm{max},m}$ (dBm)','Interpreter','latex');
ylabel('$\textsc{{ee}}^{\textsc{svd}}_{\forall}(\vec{\Phi}^{\iota},\vec{W}^\iota)$  (bits/Joule)','Interpreter','latex');
